    ...
    async fetchAllMarketData(): Promise<void> {
        ...
        const fetchPromises = Object.entries(MARKET_DATA_GROUPS).map(
            async ([groupName, sources]) => {
                try {
                    const result = await this.fetcherService.fetchFromSources(groupName, sources);
                    return result;
                } catch (error) {
                    this.logger.error(`Failed to fetch data for group ${groupName}:`, error.message);
                    return null;
                }
            }
        );
        ...
        try {
            ...
            this.logger.log(`Market data fetch completed. ${successfulResults.length}/${allResults.length} groups successful`);
            
            this.processMarketData(successfulResults);
            
        } catch (error) {
            this.logger.error('Error during scheduled market data fetch:', error.message);
        }
    }

    private processMarketData(result: FetchResult[]): void {
        //TODO: Implementar lógica para enviar dados para fila
    }
